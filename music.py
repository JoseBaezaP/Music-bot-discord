...
class music(commands.Cog):
  def __init__(self, bot):
    ...
    self.bot = bot
    self.auth_manager = SpotifyClientCredentials(os.getenv('CLIENT_ID'), os.getenv('CLIENT_SECRET'))
  ...
  def play_next(self):
    if len(self.music_quote) > 0:
      ...
      voice.play(discord.FFmpegPCMAudio(url2, **self.ffmpef_options), after=lambda e: self.play_next())
  ...
  async def info(self, ctx):
    embed = discord.Embed(title = f"{ctx.guild.name}", description = "Cree este bot por todo ya que no sirven otros bots :c"
  ...
  async def play(self, ctx, *, search):
    ...
    self.music_quote.append(url2)
    voice = discord.utils.get(self.bot.voice_clients)
    voice.play(discord.FFmpegPCMAudio(url2, **self.ffmpef_options), after=lambda e: self.play_nextYT())
  ...
  async def song(self, ctx):
      try:
        await ctx.send("Estas esuchando"+ " " + f"{self.music_quote[0]}")
      except:
